...
int my_lstat( const char *name, struct stat *st)
{
	char* s=_alloca(strlen(name)+1);
	
	strcpy (s,name);
	return lstat( s, st);
}	

#define my_stat my_lstat
...
same_name (const char *source, const char *dest)
{
  ...
  if (my_lstat (source_dirname, &source_dir_stats))
    /* Shouldn't happen.  */
    error (1, errno, "%s", source_dirname);

  if (my_lstat (dest_dirname, &dest_dir_stats))
    /* Shouldn't happen.  */
...
do_link (const char *source, const char *dest)
{
  ...
  if (!symbolic_link)
    {
      if (my_stat (source, &source_stats) != 0)
	{
  ...
  lstat_status = my_lstat (dest, &dest_stats);
  ...
  if (remove_existing_files
	 ...
	 misleading.  */
      ...
      && (!symlink || my_lstat (source, &source_stats) == 0)
  ...
  if (lstat_status == 0 || my_lstat (dest, &dest_stats) == 0)
    {
  ...
  if (optind == argc - 1)
  ...
  else if (optind == argc - 2)
    {
	 ...
	 `ln source dest/' to `ln source dest/basename(source)'.  */
	  ...
	  && my_lstat (source, &source_stats) == 0
