...
main (int argc, char **argv)
{
  ...
  if (flag_dereference)
#ifdef _WIN32
// alway lstat, because it fakes inos
    xstat = lstat;
#else
    xstat = stat;
#endif
      ...
      struct dir_list *ancestors)
{
  ...
  if (!flag_dereference && src_sb.st_nlink > 1 && earlier_file)
    {
      if (1 /* link (earlier_file, dst_path)*/)
	{
	  error (0, errno, "%s", dst_path);
	  goto un_backup;
	}
  ...
  if (S_ISFIFO (src_type))
    ...
    {
      if (1 /* mknod (dst_path, src_mode & umask_kill, src_sb.st_rdev)*/ )
	{
  ...
  if (flag_preserve)
    {
      ...
      if ( utime (dst_path, &utb) &&  errno != EACCES ) // kms: ignore error on directory
	{
	  error (0, errno, "%s", dst_path);
	  return 1;
	}
