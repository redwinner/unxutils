...
#else	
  static char const slashbase[_MAX_PATH+1] = "sort";
#endif	
...
create_temp_file (FILE **pfp)
{
  ...
  char const *temp_dir = choose_temp_base();
  static size_t temp_dir_index;	
  size_t len = strlen (temp_dir);
  struct tempnode *node =
    (struct tempnode *) xmalloc (sizeof node->next + len );
  char *file = node->name;	    
  node->next = temphead;
  if (++temp_dir_index == temp_dir_count)
    temp_dir_index = 0;
	    
	file = make_temp_file(file);
        temphead = node;
	  fprintf(stderr,"2:%s\n",file);

    
	if ( (*pfp = xfopen (file , "wb")) == NULL)
	    die (_("cannot create temporary file"), file);
...
{  
  ...
  locale = getenv ("LC_ALL");
	  if ( (locale != NULL && toupper(locale[0]) == 'C'))
		      setlocale (LC_ALL, "C");
  }	  
#endif
