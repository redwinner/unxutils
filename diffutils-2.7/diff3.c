...
#define INT_STRLEN_BOUND(type) ((sizeof(type)*CHAR_BIT - 1) * 302 / 1000 + 2)
  ...
  *ap = 0;
  ...
  {
	  mytmpfile = mktemp( mytemplat );
	  fds[1] = open( mytmpfile, _O_CREAT | _O_RDWR , _S_IREAD | _S_IWRITE );
    fdo = dup(_fileno(stdout));
    if (fds[1] != _fileno(stdout))
	{
	  dup2 (fds[1], _fileno(stdout));
	  close (fds[1]);
	}
      pid = _spawnvp ( _P_WAIT, diff_program, (char **) argv);
      if (pid < 0)
		  perror_with_exit ("execve failed");

  dup2 (fdo, _fileno(stdout));
  fd =  open( mytmpfile, _O_RDONLY , _S_IREAD | _S_IWRITE );
