...
{
  return iitoa(topdiv->get_page_offset().to_units());
}
...
{
  return iitoa(topdiv->get_page_length().to_units());
}
...
const char *vertical_position_reg::get_string()
{
  if (curdiv == topdiv && topdiv->before_first_page)
    ...
    return iitoa(curdiv->get_vertical_position().to_units());
}
...
{
  return iitoa(curdiv->get_high_water_mark().to_units());
}
...
{
  return iitoa(curdiv->distance_to_next_trap().to_units());
}
...
{
  return iitoa(topdiv->get_next_page_number());
}
...
{
  return iitoa(topdiv->get_ejecting());
}
