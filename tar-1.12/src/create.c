...
int mylstat(const char* file_Path_i,struct stat* file_Stat)
{
	...
	if (lp->ino == current_stat.st_ino && lp->dev == current_stat.st_dev)
	  {
	    ...
	    while (!absolute_names_option && *link_name == PATHSEP)
	      {
		static int warned_once = 0;
...
Read error at byte %ld, reading %d bytes, in file %s"),
      ...
      if (access (p, R_OK) == -1 && geteuid () != 0)
	{
      ...
      while (len >= 1 && namebuf[len - 1] == PATHSEP)
	len--;
      namebuf[len++] = PATHSEP;
      namebuf[len] = '\0';
	  ...
	  && parent_device != current_stat.st_dev)
	{
      ...
      tpath=_alloca(strlen(p)+1);strcpy(tpath,p);
      directory = opendir (tpath);
      if (!directory)
	{
      ...
      if (len == 2 && namebuf[0] == '.' && namebuf[1] == PATHSEP)
	len = 0;
