...
set_mode (char *file_name, struct stat *stat_info)
{
  ...
  if (!keep_old_files_option
      || (  (  S_ISUID(stat_info->st_mode) | S_ISGID(stat_info->st_mode) | S_ISVTX)))
...
chmod(file_name,S_IREAD | _S_IWRITE);
  ...
  if (we_are_root || same_owner_option)
	...
	if (we_are_root && (stat_info->st_mode & (  S_ISUID(stat_info->st_mode) | S_ISGID(stat_info->st_mode) | S_ISVTX)))
	  set_mode (file_name, stat_info);
    }
}
...
make_directories (char *file_name)
{
  ...
  for (cursor = strchr (file_name, PATHSEP);
       cursor != NULL;
       cursor = strchr (cursor + 1, PATHSEP))
    {
      /* Avoid mkdir of empty string, if leading or double PATHSEP.  */

      if (cursor == file_name || cursor[-1] == PATHSEP)
	continue;

      /* Avoid mkdir where last part of path is '.'.  */

      if (cursor[-1] == '.' && (cursor == file_name + 1 || cursor[-2] == PATHSEP))
	continue;
...
#define CURRENT_FILE_NAME (skipcrud + current_file_name)
  ...
  skipcrud = 0;
  while (!absolute_names_option && CURRENT_FILE_NAME[0] == PATHSEP)
    {
...
Attempting extraction of symbolic links as hard links")));
      ...
      if (status != 0)
	{
	  ...
	  if (CURRENT_FILE_NAME[name_length] == '.'
	      && (name_length == 0
		  || CURRENT_FILE_NAME[name_length - 1] == PATHSEP))
	    goto check_perms;
