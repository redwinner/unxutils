...
/* If FALSE is defined, we presume TRUE is defined too.  In this case,
   merely typedef my_boolean as being int.  Or else, define these all.  */
#ifndef FALSE
/* Do not use `enum my_boolean': this tag is used in SVR4 <sys/types.h>.  */
typedef enum { FALSE = 0, TRUE = 1 } my_boolean;
#else
...
my_boolean debug_set_output _((const char *));
...
struct symbol
{
  ...
  my_boolean traced;
  my_boolean shadowed;
  my_boolean macro_args;
  my_boolean blind_no_args;
...
{
  const char *name;
  my_boolean gnu_extension;
  my_boolean groks_macro_args;
  my_boolean blind_if_no_args;
  builtin_func *func;
};
...
void define_builtin _((const char *, const builtin *, symbol_lookup, my_boolean));
...
my_boolean evaluate _((const char *, eval_t *));

