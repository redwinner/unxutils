static char rcsid[] = "$Header: /cvsroot/unxutils/unxutils/rman-3.0.7/rman.c,v 1.1.1.1 2000/08/16 00:20:22 syring Exp $";
...
*/
	   ...
	   case BEGINLINE:
		...
		if (linelen>=2 && (plain[0]=='-' && plain[1]!='-' && plain[1]!=' ') /*|| plain[0]=='%' || plain[0]=='$'*/ /* not much talk of money in man pages so reasonable */) clo[clocnt++] = CurLine;
		/*
...
source_flush(void) {
  ...
  for (i=0,j=0,p=plain; i<sI && *p; i++,p++) {
    ...
    if (linelen>=LINEBREAK && c==' ') { (*fn)(ENDLINE); linelen=0; }
  }
  /* dump tags at end */
...
source_line(char *p) {
  ...
  if (!supresseol) { source_out(" "); if (finlist) source_list(); }
  supresseol=0;
  /*stagadd(ENDLINE);*/
}
...
/*	FILE *macros; -- interpret -man macros? */
	...
	static struct { char letter; int arg; char *longnames; char *desc; } option[] = {
		...
		{ 'S', 0, "source", "(ource of man page passed in)" },
		{ 'F', 0, "formatted:format", "(ormatted man page passed in)" },
...
/*		{ 'c', 0, "changeleft:changebar", "(hangebarstoleft toggle)" }, -- default is perfect */
		...
		{ 'C', 0, "TclTk", " (enable Tcl/Tk formatting)" },
...
*/
		   ...
		   case 'v': /*case '?':*/
			printf("PolyglotMan v" POLYGLOTMANVERSION " of $Date: 2000/08/16 00:20:22 $\n");
			exit(0);
